...
class ReferenceMultiTopicsConsumerImpl<T> extends ConsumerBase<T> {
    ...
    private static final Logger log = LoggerFactory.getLogger(ReferenceMultiTopicsConsumerImpl.class);
    ...
    ReferenceMultiTopicsConsumerImpl(PulsarClientImpl client, ConsumerConfigurationData<T> conf,
    ...
    ReferenceMultiTopicsConsumerImpl(PulsarClientImpl client, String singleTopic, ConsumerConfigurationData<T> conf,
                                     ExecutorProvider executorProvider, CompletableFuture<Consumer<T>> subscribeFuture, Schema<T> schema,
                                     ConsumerInterceptors<T> interceptors, boolean createTopicIfDoesNotExist) {
    ...
    ReferenceMultiTopicsConsumerImpl(PulsarClientImpl client, String singleTopic, ConsumerConfigurationData<T> conf,
        ...
        if (conf.getTopicNames().isEmpty()) {
            ...
            subscribeFuture().complete(ReferenceMultiTopicsConsumerImpl.this);
            return;
        }
    ...
    public static <T> ReferenceMultiTopicsConsumerImpl<T> createPartitionedConsumer(PulsarClientImpl client,
        ...
        ReferenceMultiTopicsConsumerImpl consumer = new ReferenceMultiTopicsConsumerImpl(client, topicName, cloneConf, executorProvider,
                future, schema, interceptors, true /* createTopicIfDoesNotExist */);

        future.thenCompose(c -> ((ReferenceMultiTopicsConsumerImpl) c).subscribeAsync(topicName, numPartitions))
